        ...
        {new Array(12).fill().map((x, i) => 
          <div className={`w-full h-20 flex justify-around ${i === currentRow ? 'activeRow' : ''}`} key={i}>
            ...
            <div className="w-[75%] h-full flex justify-center items-center gap-12 ">
                <Pion color={rows[i][0]}   onClick={() => { console.log('Color:', rows[i][0]); onClick(0) }} />
                <Pion color={rows[i][1]}  onClick={() => onClick(1)} />
                <Pion color={rows[i][2]}  onClick={() => onClick(2)} />
                <Pion color={rows[i][3]}  onClick={() => onClick(3)} />
                <Pion color={rows[i][4]}  onClick={() => onClick(4)} />
                <Pion color={rows[i][5]}  onClick={() => onClick(5)}/>
                <Pion color={rows[i][6]}  onClick={() => onClick(6)} />
                <Pion color={rows[i][7]}  onClick={() => onClick(7)} />
            </div>
            <div className="w-[20%] ">
              ...
              {hints[i] && Array.isArray(hints[i]) && (
                <div className="flex gap-4">
                  ...
                  <div key={index} className="flex justify-center items-center m-1 w-full">
